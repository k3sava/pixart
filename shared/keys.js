...
// 32 effects total; keys 1-9 map to the first 9 alphabetically, 0 to the 10th.
// The remaining 22 are reachable via the overlay nav (⌘K, /, or chevron click).
(function(){
  'use strict';
  ...
  const EFFECTS = [
    ...
    'dots','edge','film-grain','flow-field','glitch-scan','gradients',
  ...
  const CATEGORIES = [
    ...
    ['Geometric',  ['displace','distort','kaleidoscope','mosaic','superpixel','voronoi']],
    ['Cinematic',  ['bloom','crt','film-grain','neon-glow','rgb-shift']],
    ['Painterly',  ['crosshatch','ink-wash','watercolor']],
    ['Glitch',     ['glitch-scan','pixel-sort','scatter','slit-scan']],
    ...
    el.innerHTML = `
      ...
      <div class="pix-nav-overlay-panel" role="document">
        <div class="pix-nav-overlay-head">
          ...
          <input type="search" id="pix-nav-search" autocomplete="off" placeholder="search 32 effects…" aria-label="Search effects">
  ...
  function buildSplash(){
    ...
    el.innerHTML = `
      <div class="wa-splash-inner">
        ...
        <div class="wa-splash-tag">32 effects. drop an image or video. then play.</div>
        <div class="wa-splash-grid">
          <span>${numbered}</span><span>jump to first 10 effects</span>
          <span><kbd>←</kbd> <kbd>→</kbd></span><span>previous / next (32 total)</span>
