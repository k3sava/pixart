...
const EFFECTS = ['ink-wash','kaleidoscope','patterns','pixel-sort','recolor','rgb-shift','scatter'];
...
async function sampleCanvasStd(page){
  return await page.evaluate(() => {
    ...
    try {
      ...
      if(ctx2d){
        const d = ctx2d.getImageData(cx, cy, sw, sh).data;
...
async function auditEffect(browser, slug){
  ...
  try {
    ...
    // Turn animate ON via pill UI
    await page.evaluate(() => {
      const cb = document.querySelector('.wg-row[data-key="animate"] input[type=checkbox]');
      if(cb && !cb.checked) cb.click();
    });
    await page.waitForTimeout(250);

    // Collect modes from pills
    const modes = await page.$$eval('.wg-row[data-key="mode"] .wg-pill', pills => pills.map(p => p.dataset.value));

    if(modes.length === 0){
      result.modes = '✗ no mode pills found';
    } else {
      const hashes = {};
      for(const mv of modes){
        await page.evaluate((mv) => {
          const pill = document.querySelector(`.wg-row[data-key="mode"] .wg-pill[data-value="${mv}"]`);
          if(pill) pill.click();
        }, mv);
        await page.waitForTimeout(320);
        await page.evaluate(() => {
          if(window.WAEffect && window.WAEffect.pauseRender) window.WAEffect.pauseRender();
          if(window.WAEffect && window.WAEffect.renderAt) window.WAEffect.renderAt(0.5);
        });
        await page.waitForTimeout(80);
        hashes[mv] = await canvasHash(page);
      }
      ...
      // Resume render for interactive test
      await page.evaluate(() => { if(window.WAEffect && window.WAEffect.resumeRender) window.WAEffect.resumeRender(); });
    }

    // Interactive
    await page.evaluate(() => {
      const cb = document.querySelector('.wg-row[data-key="interactive"] input[type=checkbox]');
      if(cb && !cb.checked) cb.click();
    });
...
(async () => {
  ...
  for(const slug of EFFECTS){
    ...
    results.push(r);
    console.log(' ', r.slug, 'modes:', r.modes, 'int:', r.interactive, 'con:', r.console.slice(0,40));
  }
  await browser.close();

  let md = '# Sweep Batch 3 — ink-wash, kaleidoscope, patterns, pixel-sort, recolor, rgb-shift, scatter\n\n';
  md += `Date: 2026-05-13\n\n`;
  ...
  if(issues.length === 0) md += '_None — all 7 effects pass all 4 checks._\n';
  for(const i of issues) md += `- ${i.t}\n`;

  fs.writeFileSync('/Users/k3sava/projects/pixart/docs/sweep-batch-3.md', md);
  console.log('\nWrote sweep-batch-3.md with', issues.length, 'issues');
})();
