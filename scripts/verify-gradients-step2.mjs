...
await page.goto('http://localhost:8001/gradients/', { waitUntil: 'networkidle' });
...
await page.screenshot({ path: path.join(OUT, 'gradients-default.png') });
...
const breathMid = await frozenShot('breath', 0.5, 'gradients-breath.png');
const toneMid   = await frozenShot('tone',   0.5, 'gradients-tone.png');
const revealMid = await frozenShot('reveal', 0.5, 'gradients-reveal.png');

// distinctness at t=0.5 (per task)
...
console.log('revealMid', hash(revealMid));
const allDistinct = new Set([hash(breathMid), hash(toneMid), hash(revealMid)]).size === 3;
console.log('t=0.5 modes distinct?', allDistinct);

// Interactive at center
await setKey('animate', false);
...
await page.screenshot({ path: path.join(OUT, 'gradients-interactive_center.png') });
