...
await page.goto('http://localhost:8001/watercolor/', { waitUntil: 'networkidle' });
...
// 2) Default static
await page.screenshot({ path: path.join(OUT, 'watercolor-default.png') });
...
const breathMid = await frozenShot('breath', 0.5, 'watercolor-breath.png');
const bleedMid  = await frozenShot('bleed',  0.5, 'watercolor-bleed.png');
const toneMid   = await frozenShot('tone',   0.5, 'watercolor-tone.png');

// Distinctness at t=0.5
function hash(s){ return crypto.createHash('sha1').update(s).digest('hex').slice(0, 10); }
console.log('breathMid', hash(breathMid));
console.log('bleedMid',  hash(bleedMid));
console.log('toneMid',   hash(toneMid));
const distinct = new Set([hash(breathMid), hash(bleedMid), hash(toneMid)]).size === 3;
console.log('modes distinct at t=0.5?', distinct);

// Interactive center
await page.evaluate(() => window.WAEffect.pauseRender());
await setKey('animate', false);
await setKey('interactive', true);
const stage = await page.locator('#cv').boundingBox();
await page.mouse.move(stage.x + stage.width/2, stage.y + stage.height/2);
await page.waitForTimeout(250);
await page.screenshot({ path: path.join(OUT, 'watercolor-interactive_center.png') });
