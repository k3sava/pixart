...
await page.goto('http://localhost:8001/slide/', { waitUntil: 'networkidle' });
...
// Default landing — animate on, mode=orbit (default).
await page.waitForTimeout(200);
await page.screenshot({ path: path.join(OUT, 'slide-default.png') });

// Benchmark: 24 frames across the loop.
...
await setKey('mode', 'orbit');
await page.waitForTimeout(80);
const orbitMid = await frameAt(0.5);
await page.screenshot({ path: path.join(OUT, 'slide-orbit.png') });

await setKey('mode', 'tilt');
await page.waitForTimeout(80);
const tiltMid = await frameAt(0.5);
await page.screenshot({ path: path.join(OUT, 'slide-tilt.png') });

await setKey('mode', 'breath');
await page.waitForTimeout(80);
const breathMid = await frameAt(0.5);
await page.screenshot({ path: path.join(OUT, 'slide-breath.png') });
...
await page.waitForTimeout(200);
await page.screenshot({ path: path.join(OUT, 'slide-interactive_center.png') });

function sliceHash(s){ let h=0; for(let i=1000;i<5000;i+=37){ h = (h*31 + s.charCodeAt(i)) | 0; } return h; }
const ho = sliceHash(orbitMid), ht = sliceHash(tiltMid), hb = sliceHash(breathMid);
console.log('orbit', ho, 'tilt', ht, 'breath', hb);
console.log('distinct?', ho !== ht && ht !== hb && ho !== hb);
