...
await page.goto('http://localhost:8001/rgb-shift/', { waitUntil: 'networkidle' });
...
await page.screenshot({ path: path.join(OUT, 'rgb-shift-default.png') });

await setKey('mode', 'breath');
await setKey('animate', true);
await page.waitForTimeout(80);
const breathMid = await frameAt(0.5);
await page.screenshot({ path: path.join(OUT, 'rgb-shift-breath.png') });

await setKey('mode', 'orbit');
await page.waitForTimeout(80);
const orbitMid = await frameAt(0.5);
await page.screenshot({ path: path.join(OUT, 'rgb-shift-orbit.png') });

await setKey('mode', 'bloom');
await page.waitForTimeout(80);
const bloomMid = await frameAt(0.5);
await page.screenshot({ path: path.join(OUT, 'rgb-shift-bloom.png') });

await setKey('animate', false);
await setKey('interactive', true);
await page.mouse.move(640, 400);
await page.waitForTimeout(200);
await page.screenshot({ path: path.join(OUT, 'rgb-shift-interactive_center.png') });

function sliceHash(s){ let h=0; for(let i=1000;i<5000;i+=37){ h = (h*31 + s.charCodeAt(i)) | 0; } return h; }
const hb = sliceHash(breathMid), ho = sliceHash(orbitMid), hl = sliceHash(bloomMid);
console.log('breath', hb, 'orbit', ho, 'bloom', hl);
console.log('distinct?', hb !== ho && ho !== hl && hb !== hl);
