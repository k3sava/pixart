...
await page.goto('http://localhost:8001/dithering/', { waitUntil: 'networkidle' });
...
await page.evaluate(() => { document.querySelectorAll('.wa-help, .wa-help-overlay, #help-overlay, .help-overlay').forEach(e => e.remove()); });
...
await page.screenshot({ path: path.join(OUT, 'dithering-default.png'), clip: { x:0, y:0, width:1280, height:800 } });

await setKey('mode', 'grow');
await setKey('animate', true);
await page.waitForTimeout(50);
const growMid = await frameAt(0.5);
await page.screenshot({ path: path.join(OUT, 'dithering-grow.png') });

await setKey('mode', 'tone');
await page.waitForTimeout(50);
const toneMid = await frameAt(0.5);
await page.screenshot({ path: path.join(OUT, 'dithering-tone.png') });

await setKey('mode', 'threshold');
await page.waitForTimeout(50);
const thrMid = await frameAt(0.5);
await page.screenshot({ path: path.join(OUT, 'dithering-threshold.png') });

await setKey('animate', false);
await setKey('interactive', true);
await page.mouse.move(640, 400);
await page.waitForTimeout(150);
await page.screenshot({ path: path.join(OUT, 'dithering-interactive_center.png') });

function sliceHash(s){ let h=0; for(let i=1000;i<5000;i+=37){ h = (h*31 + s.charCodeAt(i)) | 0; } return h; }
console.log('growHash', sliceHash(growMid), 'toneHash', sliceHash(toneMid), 'thrHash', sliceHash(thrMid));
console.log('grow==tone?', growMid === toneMid, 'tone==thr?', toneMid === thrMid, 'grow==thr?', growMid === thrMid);
