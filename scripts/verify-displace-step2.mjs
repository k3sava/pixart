...
const bench = await page.evaluate(async () => {
  // ensure animate-mode set to swell for the bench so it does real work
  await new Promise(r => requestAnimationFrame(r));
...
// 2) Default static frame (animate off, interactive off)
await page.screenshot({ path: path.join(OUT, 'displace-default.png') });

// Helper: render at frozen t01 with animation paused, then screenshot.
...
const swellMid  = await frozenShot('swell',  0.5, 'displace-swell.png');
const toneMid   = await frozenShot('tone',   0.5, 'displace-tone.png');
const breathMid = await frozenShot('breath', 0.5, 'displace-breath.png');

// 6) Distinctness at t=0 too (animation must be paused so capture is stable)
await page.evaluate(() => window.WAEffect.pauseRender());
await setKey('mode', 'swell');
const swellZero = await frameAt(0);
await setKey('mode', 'tone');
const toneZero = await frameAt(0);
await setKey('mode', 'breath');
const breathZero = await frameAt(0);

// 7) Disable animate, enable interactive, move cursor to center
await setKey('animate', false);
await setKey('interactive', true);
await page.mouse.move(640, 400);
await page.waitForTimeout(150);
await page.screenshot({ path: path.join(OUT, 'displace-interactive_center.png') });

function hash(s){ return crypto.createHash('sha1').update(s).digest('hex').slice(0, 10); }
console.log('swellMid', hash(swellMid));
console.log('toneMid',  hash(toneMid));
console.log('breathMid',hash(breathMid));
console.log('swellZero',hash(swellZero));
console.log('toneZero', hash(toneZero));
console.log('breathZero',hash(breathZero));
const allDistinct = new Set([hash(swellZero), hash(toneZero), hash(breathZero)]).size === 3;
console.log('t=0 modes distinct?', allDistinct);
